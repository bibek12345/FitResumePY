...
def create_job_posting(payload: JobPostingCreate):
    with session_scope() as session:
        ...
        if payload.company_name:
            company = crud.get_or_create_company(session, payload.company_name)
...
def tailor_resume(request: TailorRequest):
    with session_scope() as session:
        resume = crud.get_resume(session, request.resume_id)
        job = crud.get_job_posting(session, request.job_posting_id)
...
def import_jobs_from_csv(content: bytes | str) -> list[int]:
    ...
    with session_scope() as session:
        for row in reader:
            ...
            if company_name:
                company = crud.get_or_create_company(session, company_name)
            job = crud.create_job_posting(
                session,
                title=title,
                company=company,
